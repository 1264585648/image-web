...
- 已接入项目的前端工作台：上传、模板、生成、合规检测、历史结果
- 高保真静态原型：桌面端首页、桌面端工作台、手机端首页、手机端工作台
...
- `frontend/index.html`：ProductShot AI 首页 + 工作台
- `frontend/styles.css`：响应式高保真样式
- `frontend/app.js`：真实接入后端 API 的交互逻辑
- `frontend/README.md`：前端接入说明
...
- `index.html`：可直接打开的高保真静态原型，偏设计稿展示
...
当前模板已经绑定平台规则集，生成结果会返回增强版 `compliance` 报告：

- Amazon / Temu / Shopify / Universal / Mobile Commerce 分别使用不同规则
- 报告包含规则名称、分项检查、指标值、警告和修改建议
- 保留旧版 `score`、`checks`、`metrics`、`warnings` 字段，兼容现有前端
- OCR、文字、水印、Logo 一致性质检已预留 `qc_status` 接口，本期默认返回 `not_run`
